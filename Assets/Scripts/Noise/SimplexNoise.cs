...
namespace SimplexNoise
{
    ...
    public static class Noise
    {
        ...
        public static float[,,] Calc3D(int width, int height, int length, float scale, int offsetX, int offsetY, int offsetZ)
        {
            var values = new float[width, height, length];
            for (var i = 0; i < width; i++)
                for (var j = 0; j < height; j++)
                    for (var k = 0; k < length; k++)
                        values[i, j, k] = Generate((i + offsetX) * scale, (j + offsetY) * scale, (k + offsetZ) * scale) * 128 + 128;
            return values;
        }
        ...
        public static float CalcPixel3D(int x, int y, int z, float scale, int offsetX, int offsetZ)
        {
            return Generate((x + offsetX) * scale, y * scale, (z + offsetZ) * scale) * 128 + 128;
        }
